...
class LaserMerge {
    ...
    public:
        ...
        void ScanCallback(const sensor_msgs::LaserScan::ConstPtr& front_msg,const sensor_msgs::LaserScan::ConstPtr& back_msg) {
            ROS_WARN("Received front and back scans");
            ...
            ROS_WARN("Printing filtered_front_ and filtered_back_ sizes before filtering: %zu, %zu", filtered_front_.size(), filtered_back_.size());
            // // // After filtering ranges, resize to a fixed size:
            // filtered_front_.resize(541, 0.0);  // Pad with 0s if needed
            // filtered_back_.resize(541, 0.0);
            ...
            back_scan_msg_.angle_min = -1.57079632679;//gle min for back scan, taken from python code
            back_scan_msg_.angle_max =1.57079632679; 
